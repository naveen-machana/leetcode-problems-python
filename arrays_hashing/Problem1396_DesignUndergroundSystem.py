...
class UndergroundSystem:
    ...
    def __init__(self):
        self.checkins = {}  # (id -> (startingStation, time))
        self.totalTime = {}  # (startingStation, endingStation) -> (totalTimeSoFar, count)
    def checkIn(self, id: int, startingStation: str, t: int) -> None:
        self.checkins[id] = (startingStation, t)

    def checkOut(self, id: int, endingStation: str, endsAt: int) -> None:
        startingStation, startedAt = self.checkins[id]
        route = (startingStation, endingStation)
        if route not in self.totalTime:
            self.totalTime[route] = [0, 0]
        self.totalTime[route][0] += (endsAt - startedAt)
        self.totalTime[route][1] += 1
    def getAverageTime(self, startStation: str, endStation: str) -> float:
        route = (startStation, endStation)
        return self.totalTime[route][0]/self.totalTime[route][1]